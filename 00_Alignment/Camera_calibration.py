...
imgpoints3D = ( list( map( lambda pts: np.asarray(cv2.convertPointsToHomogeneous(pts)).reshape(objpoints[0].shape) , imgpoints )) )
for i in range(len( imgpoints3D)):
    imgpoints3D[i][:,2] = 0
print(imgpoints3D[0].shape)
print(imgpoints3D[0][:,2])

print(imgpoints.shape)
objpoints2D = list(map( lambda pts: pts[:, :-1] , objpoints))
objpoints2D = np.asarray(objpoints2D, np.float32)
print(objpoints2D.shape)

print(objpoints[0].shape)
#
ret, K, dist, rvecs, tvecs = cv2.calibrateCamera(imgpoints3D, objpoints2D, (480, 640),None,None)
np.save("./camera_calibration/00_allThermal/calib_params/ret_inv", ret)
np.save("./camera_calibration/00_allThermal/calib_params/K_inv", K)
np.save("./camera_calibration/00_allThermal/calib_params/dist_inv", dist)
np.save("./camera_calibration/00_allThermal/calib_params/rvecs_inv", rvecs)
np.save("./camera_calibration/00_allThermal/calib_params/tvecs_inv", tvecs)
#
